...
exports.userConnection = exports.userUpdateById = exports.userCreateOne = exports.userFindById = void 0;
...
const validatedInputResolver = (next) => async (rp) => {
    const { record: { email, no_hp }, } = rp.args;
    console.log(email);
    if (email) {
    ...
    if (no_hp) {
        if (!no_hp.match(handPhoneRegex))
            throw new Error(' phone number is not valid');
    }
    return next(rp);
};
exports.userFindById = user_1.UserTC.mongooseResolvers.findById();
exports.userCreateOne = user_1.UserTC.mongooseResolvers.createOne().wrapResolve(validatedInputResolver);
exports.userUpdateById = user_1.UserTC.mongooseResolvers.updateById().wrapResolve(validatedInputResolver);
exports.userConnection = user_1.UserTC.mongooseResolvers
    .connection({
    defaultLimit: 10,
})
    .removeArg(['filter'])
